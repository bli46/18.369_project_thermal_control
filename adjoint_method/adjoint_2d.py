...
def sym(arrx_in,arry_in,arrz_in):
    """Helper function for fdtd function"""
    arrx = arrx_in.copy()
    arry = arry_in.copy()
    arrz = arrz_in.copy()
...
def fdtd(epsx,epsy,epsz,
         ...
         src_intg=True,
         xphase=0,yphase=0):
    '''Runs the meep simulations and returns fourier-transformed ez fields.
    Main edit compared to the solver2d.py version is that the stopping condition
    is measured at the end of the grid instead of in the middle.'''
    dpml=npml*du
    Lx=nx*du
    Ly=ny*du
    resolution=1./du
    ...
    if monitor_comp==2:
        mtr_c=mp.Ez
    mtr_r=mp.Vector3((nx/2-npml-1)*du,0,0)
    mtr_tol=1e-6
    def epsfun(r):
...
def adjoint_compute(grad_ez, fcen, df, dt, nx, ny, eps, ez):
    '''
    computes adjoint gradient given gradient of the objective with respect to ez
    '''
    courant = 0.5
    res = 40.
    du = 1./res
    ...
    adjz = np.conj(grad_ez) * (df/(expfac*iw))
    n0 = 1.
    pml_direction = mp.X

    uz,dt = fdtd(eps,eps,eps,
                          np.zeros((nx,ny)),
                          np.zeros((nx,ny)),
                          adjz,
                          fcen,df,courant,
