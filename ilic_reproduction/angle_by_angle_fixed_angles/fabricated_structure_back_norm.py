...
@timeout(300)
def perform_tasks(n, wvl, wvl_number):
    fcen = 1./wvl
    df = .01*fcen
    # set pml
    dpml = 3.
    ...
    with open('back_fabricated_structure_reflectance_angle_' + str(n) + '_wavelength_number_' + str(wvl_number)+'.npy', 'wb') as f:
        np.save(f, flux_wvl)
        np.save(f, normalized_refl_flux)
        np.save(f, np.array(stack_refl_flux))
        np.save(f, np.array(norm_tran_flux))
        
def perform_tasks_in_time(n, wvl, wvl_number):
    fcen = 1./wvl
    df = .01*fcen
    # set pml
    dpml = 3.
    ...
    dT = 10.*1/fcen
    decay = 1e-6
    sim.run(until_after_sources=150000)
    norm_refl_data = sim.get_flux_data(refl)
    norm_tran_flux = mp.get_fluxes(tran)
    sim.reset_meep()
    ########################################
...
for i, wvl in enumerate(wavelengths):
    if flag == False:
        try:
            perform_tasks(n, wvl, i)
        except:
            perform_tasks_in_time(n, wvl, i)
            flag = True
    else:
        perform_tasks_in_time(n, wvl, i)
